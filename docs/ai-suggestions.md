...
A conexão pertence ao usuário autenticado, dura 30 dias e sobrevive a logout, recarga e outros navegadores. A chave fica criptografada em tabela privada, usando AES-256-GCM e contexto `gemini:userId:connectionId`, com `WEBFLOW_TOKEN_ENCRYPTION_KEY`. Não existe chave de IA compartilhada do ReplaceAll. Não usar localStorage, cookies ou logs para credenciais. A chave nunca retorna ao navegador depois de salva. O servidor faz as chamadas Gemini com a credencial do próprio usuário. Infraestrutura do app ainda tem custos próprios; isso não transfere tokens para uma conta do ReplaceAll.
...
O resultado preenche diretamente apenas a ocorrência escolhida. Escrita no CMS mantém prévia, validação, confirmação e auditoria. Variables continuam protegidos. HTML/Rich Text é tratado pelo substituidor existente, não pela IA.
