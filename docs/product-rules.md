...
Current contract consolidated on 2026-09-23. The brand is **ReplaceAll**; CopyReplace/Universal Values identifiers remain where compatibility requires them.

## Scope and user control

The product finds supported CMS content and static-page content available through the active Webflow Designer extension. It does not crawl the whole published site, read embeds, validate SEO/JSON-LD or check broken links. SEO is coming soon. Global Facts is a versioned reference, not an automatic site audit.
...
- Manual review flags are separate from application and Managed Values. Specific-term scans start pending rather than inheriting unrelated earlier reviews. Never calculate counters by naive subtraction from all raw detections.
...
See [scans](scans.md), [CMS changes](cms-changes.md), [Managed Values](managed-value-sync.md), [Designer](designer-dashboard.md), [AI](ai-suggestions.md) and [limits](free-plan.md) for feature contracts.
