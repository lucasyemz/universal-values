...
O OAuth usa state aleatório, cookie HttpOnly/SameSite=Lax, expiração e vínculo com o usuário e workspace. Uma claim atômica no banco impede duas trocas do mesmo código. Apenas um callback chega ao provedor; repetições concluídas retornam à conexão existente. A autorização no Webflow e o checkbox na aplicação representam o consentimento para armazenar a conexão.
