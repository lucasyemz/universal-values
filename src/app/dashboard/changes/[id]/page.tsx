...
export default async function ChangePage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  return <main className="ui-page">
    <SiteContext siteName={site.display_name} title={request.reverts_request_id ? t("Reversão de alterações") : t("Alterações no CMS")} siteId={request.site_id} workspaceId={request.workspace_id} />
    <FreshLink href={back}>← {managed ? t("Voltar ao Managed Value") : t("Voltar ao scan")}</FreshLink>
    ...
    {request.reverts_request_id || managed ? <ul className="mt-6 space-y-4">{plan.map((field, index) => <li key={field.sourceKey} className="rounded border bg-white p-5"><h2 className="font-semibold">{field.occurrence.collection_name} → {field.occurrence.item_name} → {field.occurrence.field_name}</h2><p className="mt-1 text-sm">Locale: {field.occurrence.locale || t("padrão")}</p><Diff before={typeof field.before === "string" ? field.before : JSON.stringify(field.before, null, 2)} after={typeof field.after === "string" ? field.after : JSON.stringify(field.after, null, 2)} />{request.results[index] && <p className="mt-3 text-sm"><StatusBadge status={request.results[index]!.status} /> {t(request.results[index]!.message)}</p>}</li>)}</ul> : <ul className="mt-6 space-y-4">{request.changes.map((change) => {
      ...
      return <li key={change.occurrenceId} className="rounded border bg-white p-5"><h2 className="font-semibold">{occurrencePresentation(o).title}</h2><p className="mt-1 text-sm text-faint">{o.collection_name} → {o.item_name} → {o.field_name} · locale {o.locale || t("padrão")}  {t("· posição")} {o.start_pos}</p><Diff before={editableValue(o.canonical)} after={replacementLabel(change.after)} />{o.canonical.type === "image" && change.after.type === "image" && <ImageChangePreview before={o.canonical.url} after={change.after.url} />}{result && <p className="mt-3 text-sm"><StatusBadge status={result.status} /> {t(result.message)}</p>}</li>;
    })}</ul>}
