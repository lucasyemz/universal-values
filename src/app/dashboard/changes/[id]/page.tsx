...
export default async function ChangePage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  return <main className="ui-page">
    ...
    <Link className="text-accent underline" href={"/dashboard/scans/" + request.scan_id}>← Voltar ao scan</Link>
    ...
    <Notice title={request.status === "preview" ? "Nada foi aplicado nesta prévia" : "Publicação sob seu controle"}>As mudanças ficam no conteúdo preparado do CMS. O site não será publicado. Campos alterados desde o scan serão bloqueados; outros campos podem ser aplicados mesmo que algum falhe.</Notice>
    {request.status === "preview" && <p className="mt-3 text-sm text-muted">Evite editar estes campos no Webflow durante a aplicação. Valores já gerenciados mantêm o cadastro anterior no dashboard; faça outro scan após esta edição pontual.</p>}
