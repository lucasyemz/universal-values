...
export default async function ValuePreviewPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  return <main className="ui-page">
    <SiteContext title="Revisar Managed Value" siteId={preview.site_id} />
    <Link className="text-accent" href={"/dashboard/scans/" + preview.scan_id}>← Sugestões do scan</Link>
    <PageHeader title="Revisar Managed Value" description="Confira o valor e as origens que serão vinculadas." />
    <section className="mt-8 ui-card p-6">
      ...
      <p className="mt-4 leading-7 text-muted">Serão criados o valor e seus vínculos com as fontes abaixo, usando o conteúdo observado no scan. Esta etapa organiza as fontes e não altera o Webflow. A sincronização deste cadastro ainda não está disponível.</p>
      ...
      {preview.managed_value_id ? <Link className="mt-6 inline-block text-accent underline" href={"/dashboard/managed-values/" + preview.managed_value_id}>Abrir Managed Value criado</Link> : expired ? <p className="mt-6 text-amber-800">Prévia expirada. Prepare uma nova seleção.</p> :
        <form action={confirmManagedValue} className="ui-action-bar mt-6 space-y-4"><input type="hidden" name="id" value={id} /><label className="flex gap-3"><input type="checkbox" name="confirmed" value="yes" required />Confirmo que estas fontes representam o mesmo dado de negócio.</label><SubmitButton pendingLabel="Criando Managed Value…">Criar Managed Value</SubmitButton></form>}
    </section>
  </main>;
}
