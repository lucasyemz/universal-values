...
export default async function PlanPage({ searchParams }: { searchParams: Promise<{ target?: string; error?: string; changed?: string }> }) {
  ...
  const rows = [
    ...
    { label: t("Operações ativas e na fila"), used: usage.active, limit: null, detail: t("Até 20 alterações CMS confirmadas na fila. Execução sequencial; scans mantêm seus limites.") },
  ...
  return <main className="ui-page">
    <PageHeader title={t("Plano e consumo")} description={t("Consulte seus limites e gerencie o plano desta conta.")} actions={<FreshLink href="/dashboard/plan">{t("Atualizar consumo")}</FreshLink>} />
    ...
    {admin && <p className="mb-4 text-sm text-muted">{usage.trackingSince ? t("Uso Free e Administrador registrado neste mês. A contagem do Administrador começou em {0}; atividades anteriores não estão incluídas. Sites e operações ativas mostram o total atual.", new Date(usage.trackingSince).toLocaleString(t.dateLocale)) : t("Contagem do Administrador ainda não habilitada. Os números abaixo incluem apenas consumo Free e totais atuais.")}</p>}
    <div className="mb-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{rows.map(row => <Card key={row.label}><h3 className="text-sm font-medium">{t(row.label)}</h3><p className="my-3 text-2xl font-semibold">{row.used} <span className="text-sm font-normal text-muted">{row.limit === null ? "" : `/ ${admin ? t("sem cota") : row.limit}`}</span></p>{!admin && row.limit !== null && <Progress value={Math.min(row.used, row.limit)} max={row.limit} label={`${t(row.label)}: ${row.used} / ${row.limit}`} />}<p className="mt-2 text-xs text-muted">{t(row.detail)}</p></Card>)}</div>
