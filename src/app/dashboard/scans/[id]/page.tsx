...
export default async function ScanPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string; filter?: string; q?: string; group?: string; operation?: string; operationError?: string; page?: string }> }) {
  ...
  return <main className="ui-page">
    ...
    <PageHeader title={scan.status === "preview" ? t("Revisar scan") : t("Revisão do scan")} description={scan.status === "preview" ? t("Confira o que será lido antes de iniciar.") : t("Revise as ocorrências e mantenha o foco no que precisa mudar.")} status={<StatusBadge status={displayStatus} />} />
    ...
    {scan.status === "preview" ? <section className="mt-8 ui-card p-6">
      ...
      <p role="status" className="mt-6">{scan.status === "cancelled" ? t("Scan cancelado.") : scan.status === "limited" ? t("Scan finalizado com cobertura parcial.") : t(displayStatus === "scanned" ? "Escaneado. Revise as ocorrências pendentes." : "Scan concluído.")} </p>}
    ...
    {["completed","limited"].includes(scan.status) && <section className="mt-10">
      ...
      {filter !== "variables" && <>
      {groupKey && <p className="mt-3 text-sm">{t("Exibindo o grupo salvo selecionado.")} <Link prefetch={false} className="text-accent underline" href={scanHref + "?filter=" + filter}>{t("Ver todos os resultados do scan")}</Link></p>}
