...
export default async function ChangesPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ page?: string; filter?: string }> }) {
  ...
  return <SitePage site={view.site} title={t("Alterações")} description={t("Consulte prévias e operações do CMS e das páginas estáticas.")}>
    <nav className="ui-tabs mb-6" aria-label={t("Filtrar alterações")}>{([['all',t("Todas")],['cms','CMS'],['static',t("Páginas estáticas")],['attention',t("Precisam de atenção")]] as const).map(([value,label])=><Link key={value} className="ui-tab" aria-current={filter===value?'page':undefined} href={base+'?filter='+value}>{label}</Link>)}</nav>
