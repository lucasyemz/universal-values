...
export default async function SitePreviewPage({ params, searchParams }: {
  ...
  return <main className="ui-page !max-w-2xl">
    <Link className="text-sm text-accent" href={"/dashboard/connections/" + preview.connection_id}>← Escolher site</Link>
    ...
    <section className="mt-6 ui-card p-6">
      ...
      {preview.site_id ? <Link className="mt-6 inline-block text-accent underline" href={"/dashboard/sites/" + preview.site_id}>Vínculo concluído. Explorar CMS</Link> : preview.expired ? <p role="alert" className="mt-6 text-amber-800">A prévia expirou. Volte e gere uma nova.</p> :
