...
export default async function SitePreviewPage({ params, searchParams }: {
  ...
  return <main className="ui-page !max-w-2xl">
    ...
    <section className="mt-6 ui-card p-6">
      ...
      {preview.site_id ? <Link className="ui-btn ui-btn-primary mt-6" href={"/dashboard/sites/" + preview.site_id + "/scans"}>Vínculo concluído. Abrir scans</Link> : preview.expired ? <p role="alert" className="mt-6 text-amber-800">A prévia expirou. Volte e gere uma nova.</p> :
