...
import { DiscoverSites } from "@/components/sites/discover-sites";
import { loadWorkspaceSites } from "@/modules/sites/service";
import { startWebflowConnection, revokeWebflowAccess } from "@/modules/sites/actions";
...
export default async function Settings({params,searchParams}: {params: Promise<{workspaceId:string}>;searchParams:Promise<{sites?:string;error?:string;revoked?:string}>}) {
 ...
 <section className="ui-card mt-6 p-6"><h2 className="text-lg font-semibold">{t("Sites e coleções")}</h2><p className="mt-2 text-sm text-muted">{t("Atualize as coleções na preparação do scan. Para incluir outro site, use Conectar Webflow.")}</p><div className="mt-4 flex flex-wrap gap-3">{view.sites.map(site=><Link key={site.id} className="ui-btn" href={"/dashboard/sites/"+site.id+"/scans/new"}>{site.display_name}  {t("· Escolher coleções")}</Link>)}</div>
 {!!view.connections.length&&<DiscoverSites workspaceId={workspaceId} linked={view.sites.map(s=>s.webflow_site_id)}/> }</section>
