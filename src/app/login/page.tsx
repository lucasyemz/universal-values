    ...
    <main className="grid min-h-screen bg-surface lg:grid-cols-2">
      ...
      {!configured ? <Notice tone="warning" title="Configuração de acesso pendente">Configure as variáveis do Supabase para habilitar o login.</Notice> : (
        <form action={login} className="mt-8 space-y-5">
          ...
          <label className="block text-sm font-medium">E-mail<input className="mt-2 block w-full rounded border border-line p-3" type="email" name="email" autoComplete="username" required maxLength={254} /></label>
          <label className="block text-sm font-medium">Senha<input className="mt-2 block w-full rounded border border-line p-3" type="password" name="password" autoComplete="current-password" required maxLength={1024} /></label>
          <SubmitButton className="w-full" pendingLabel="Entrando…">Entrar<ArrowRight size={16} /></SubmitButton>
          <p className="text-xs leading-5 text-muted">Esta ação inicia uma sessão neste navegador.</p>
        </form>
      )}
      </div></section>
    </main>
