...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return (
    <main className="grid min-h-screen lg:grid-cols-2">
      <section className="hidden flex-col justify-between border-r bg-accent-soft p-12 lg:flex"><Link href="/"><Brand /></Link><div className="max-w-lg"><ShieldCheck size={40} className="mb-8 text-accent" /><h2 className="text-4xl font-semibold leading-tight tracking-tight">Tudo no lugar.<br />Cada mudança, revisada.</h2><p className="mt-6 text-base leading-8 text-muted">Um espaço para encontrar, organizar e atualizar as informações que se repetem no seu CMS Webflow.</p></div><p className="text-xs text-muted">Universal Values · Seu conteúdo, sob seu controle.</p></section>
