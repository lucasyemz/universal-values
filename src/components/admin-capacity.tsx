...
export async function AdminCapacity() {
  ...
  return <>
    <SectionHeader title={t("Capacidade do ReplaceAll")} description={t("Saldo compartilhado do serviço Free. Estes limites internos não representam a cota dos provedores nem limitam o Administrador.")} />
    ...
    <div className="mb-8 grid gap-4 lg:grid-cols-2">
      ...
      <Card><h3 className="font-semibold">Gemini · Google</h3><p className="my-3 text-sm text-muted">{t("A chave de geração não fornece o saldo do projeto Google. Consulte limites por modelo, requisições e tokens no AI Studio. A cota de gerações do ReplaceAll aparece separadamente abaixo.")}</p><a className="ui-btn" href="https://aistudio.google.com/usage" target="_blank" rel="noreferrer">{t("Ver consumo no AI Studio")}</a></Card>
