...
export function AiBatch({ scanId, groupId, children, selectionScoped = false }: { selectionScoped?: boolean; scanId: string; groupId: string; children: ReactNode }) {
  ...
  return <Context.Provider value={{ register, running, groupId }}>
    {(count > 0 || progress) && <section className="ui-card mt-6 p-4" aria-label={t(selectionScoped ? "Preencher seleção com IA" : "Preencher este grupo com IA")}>
      <button type="button" className="ui-btn disabled:opacity-40 disabled:cursor-not-allowed" disabled={work?.busy || running || ai.loading || !count} onClick={() => { if (ai.configured) void generate(); else setConnecting(true); }}><Sparkles size={16} aria-hidden="true" />{t(selectionScoped ? "Preencher seleção com IA" : "Preencher este grupo com IA")} ({count})</button>
      <p className="mt-2 text-xs text-muted">{t(selectionScoped ? "Gera sugestões apenas para os textos de exemplo selecionados, até 20 por lote. Preserva rascunhos e Managed Values." : "Preenche até 20 textos de exemplo deste grupo por lote, usando o contexto de cada item. Preserva edições e Managed Values. As demais ações ficam bloqueadas durante a geração.")}</p>
