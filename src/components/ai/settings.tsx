...
export function AiSettings({ onConnected }: { onConnected?: () => void } = {}) {
  ...
  return <section className="ui-card max-w-2xl p-6"><h2 className="text-lg font-semibold">{t("Gemini · Sugestões de texto")}</h2>
    ...
    <p className="mt-3 text-sm leading-6 text-muted">{t("Use um projeto sem faturamento para acessar a faixa gratuita do Google. O CopyReplace não fornece créditos nem usa uma chave paga quando sua cota termina. Se você ativar faturamento, as cobranças serão da sua conta Google.")}</p>
