...
export function GeminiUsage() {
  ...
  return <section className="mb-8" aria-label="Gemini">
    <SectionHeader title="Gemini" description={t("Cota de geração no Copyreplace, separada dos limites do plano.")} action={<button className="ui-btn" disabled={loading} onClick={()=>{setLoading(true);setRevision(value=>value+1);}}>{loading?t("Atualizando…"):t("Atualizar consumo")}</button>} />
    ...
    {usage && <>
      ...
      <p className="mt-3 text-xs text-muted">{t("Este limite do Copyreplace também vale para administradores. Cada pedido autorizado consome uma geração, mesmo se falhar ou for interrompido. Consultar o consumo não gasta cota.")}</p>
    </>}
    <p className="mt-3 text-sm text-muted">{t("A cota do Google é independente e pode ser atingida antes. O Copyreplace não consulta o saldo de tokens nem a cota restante do seu projeto Google.")} <a className="text-accent underline" href="https://aistudio.google.com/" target="_blank" rel="noopener noreferrer">Google AI Studio ↗</a></p>
  </section>;
}
