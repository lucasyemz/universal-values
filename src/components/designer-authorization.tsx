...
export function DesignerAuthorization({ id, siteId, name }: { id: string; siteId: string; name: string }) {
  ...
  return <section className="ui-card p-6"><h2 className="text-lg font-semibold">Conectar a extensão</h2>
    <p className="mt-2 text-sm text-muted">Autorizar por 30 dias o acesso ao histórico e o registro de prévias e resultados de páginas estáticas de <strong>{name}</strong>. As alterações continuam exigindo confirmação dentro do Designer.</p>
