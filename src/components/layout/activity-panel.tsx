...
export function ActivityPanel({ userId }: { userId: string }) {
  ...
  const pending = items.filter(item => item.state !== "done").length;
  if (!items.length) return null;
  return <aside aria-label="Central de processos" className="fixed bottom-4 right-4 z-30 max-w-[calc(100vw-2rem)]">
    {open && <section id="activity-panel" aria-labelledby="activity-title" onKeyDown={event => { if (event.key === "Escape") { event.stopPropagation(); toggle(false); } }} className="ui-card mb-3 flex max-h-[min(32rem,70dvh)] w-96 max-w-full flex-col overflow-hidden shadow-xl">
      <div className="flex items-center justify-between border-b p-4"><div><h2 id="activity-title" className="font-semibold">Seus processos</h2><p className="text-xs text-muted">Acompanhe enquanto navega</p></div><button type="button" className="ui-btn ui-btn-ghost" aria-label="Minimizar processos" onClick={() => toggle(false)}><ChevronDown size={18} /></button></div>
      <div className="overflow-y-auto p-4">
        {error && <p role="status" className="mb-3 text-sm text-amber-800">{error}</p>}
        {items.some(i => i.kind === "change" && i.state === "active") && worker !== "online" && <p className="mb-3 rounded-lg bg-subtle p-3 text-xs text-amber-800">{worker === "offline" ? "Worker sem sinal recente. As alterações continuam salvas na fila." : "Não foi possível verificar a disponibilidade do worker."}</p>}
        <ul className="space-y-3">{items.map(item => <li key={`${item.kind}:${item.id}`} className="rounded-xl border p-3">
          <div className="flex items-start gap-2">{item.state === "attention" ? <TriangleAlert size={17} className="mt-1 shrink-0 text-amber-700" aria-hidden="true" /> : item.state === "done" ? <CircleCheck size={17} className="mt-1 shrink-0 text-accent" aria-hidden="true" /> : <LoaderCircle size={17} className="mt-1 shrink-0 motion-safe:animate-spin text-accent" aria-hidden="true" />}<div className="min-w-0"><p className="text-sm font-semibold">{item.title}</p><p className="truncate text-xs text-muted">{item.site}</p></div></div>
          <p className="mt-3 text-xs font-medium">{item.label}</p><p className="mt-1 text-xs text-muted">{item.detail}</p>
          {item.total !== undefined && <Progress value={item.current} max={item.total} label="Campos processados" />}
          {item.kind === "scan" && item.state !== "done" && <p className="mt-2 text-xs text-muted">O scan depende da página de execução aberta. Abra os detalhes para acompanhar ou retomar.</p>}
          <Link className="mt-3 inline-block text-sm font-medium text-accent underline underline-offset-4" href={item.href}>Ver detalhes</Link>
        </li>)}</ul>
        {limited && <p className="mt-3 text-xs text-muted">Exibindo até 50 operações de cada tipo. Consulte o histórico do site para ver mais.</p>}
      </div>
    </section>}
    <div className="flex justify-end"><button ref={trigger} type="button" aria-expanded={open} aria-controls={open ? "activity-panel" : undefined} onClick={() => toggle(!open)} className="ui-btn ui-btn-primary rounded-full shadow-lg"><ActivityIcon size={18} aria-hidden="true" />Processos{pending > 0 && <span className="rounded-full bg-white/20 px-2 tabular-nums">{pending}</span>}{error && <span aria-label="Atualização indisponível">!</span>}</button></div>
  </aside>;
}
