...
export function ManagedValueEditor({ valueId, version, canonical, disabled }: { id: string; valueId: string; version: number; canonical: ManagedValue; disabled: boolean }) {
  ...
  return <section className="ui-card mt-6 space-y-4 p-6">
    ...
    <textarea id="managed-replacement" value={replacement} onChange={event=>setReplacement(event.target.value)} disabled={disabled||pending} maxLength={10000} rows={canonical.type==="text"?3:1} className="w-full rounded border p-3 text-sm" aria-describedby="managed-hint"/>
    <p id="managed-hint" className="text-xs text-muted">{canonical.type==="text"?t("Texto desejado, sem deixar vazio."):t(inputHints[canonical.type])}</p>
