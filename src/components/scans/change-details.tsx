...
export async function ChangeDetails({ id, error, scanId }: { id: string; error?: string; scanId?: string }) {
  ...
  const content = <section className={scanId ? "ui-card mt-5 p-5" : "ui-page"}>
    ...
    {!scanId && <FreshLink href={back}>← {managed ? t("Voltar à Variável") : t("Voltar ao scan")}</FreshLink>}
    ...
    {managed && <Notice title={t("Sincronização da Variável")}>{t("A confirmação define o valor central desejado. Cada fonte é sincronizada separadamente, com releitura e auditoria. Fontes que falharem continuam pendentes; cancelar o restante não desfaz o valor central nem os campos aplicados.")}</Notice>}
    {!managed && request.status === "preview" && <p className="mt-3 text-sm text-muted">{t("Evite editar estes campos no Webflow durante a aplicação. Campos vinculados a Variáveis são protegidos contra edições pontuais.")}</p>}
    ...
    {request.status === "completed" && !request.reverts_request_id && <Notice title={t("Continue sua revisão")}><p>{t("Ocorrências aplicadas ou já atualizadas são marcadas como revisadas automaticamente. Falhas e conflitos precisam da sua atenção.")}</p><FreshLink href={back} className="mt-3">{managed ? t("Voltar à Variável atualizada") : t("Continuar nos pendentes")}</FreshLink></Notice>}
