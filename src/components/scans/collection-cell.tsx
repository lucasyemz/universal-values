...
export function ScanCollectionCell({scan,href}:{scan:Pick<Scan,"plan">;href:string}) {
  ...
  return <div ref={root} className="relative flex min-w-0 max-w-xs items-center gap-1.5" onBlur={event=>{if(!event.currentTarget.contains(event.relatedTarget))setOpen(false);}}>
    <Link prefetch={false} href={href} title={first.name} className="relative z-10 min-w-0 truncate rounded-md border bg-subtle px-2 py-0.5 text-xs font-medium text-muted focus-visible:outline-2 focus-visible:outline-accent">{first.name}</Link>
    {others.length>0 && <>
