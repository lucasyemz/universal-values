...
export async function ScanCollectionSummary({scan,reviewCount}:{scan:Scan;reviewCount:number}) {
 ...
 return <details className="ui-card mt-4 p-4">
  <summary className="cursor-pointer text-sm font-semibold">{t("Coleções lidas ({0})",collections.filter(c=>c.state!=="unread").length)} · {t("{0} ocorrências na revisão",reviewCount)}</summary>
