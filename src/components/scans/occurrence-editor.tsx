...
export function OccurrenceEditor({ rows, scanId, reviewedIds = [], editableBoundOccurrenceIds = [], linkedValues = {} }: { rows: Row[]; scanId: string; reviewedIds?: string[]; editableBoundOccurrenceIds?: string[]; linkedValues?: Record<string, { id: string; name: string; divergence?: boolean; bindingId?: string }> }) {
  ...
  return <div className="mt-5 space-y-5">
    <div className="rounded-xl bg-subtle p-5">
      <p className="mb-1 text-[11px] font-semibold uppercase tracking-wider text-muted">Valor encontrado · {rows.length} ocorrências · {occurrences.length} livres para edição</p>
      <h3 className="mb-5 break-all text-lg font-semibold">{editableValue(rows[0]!.occurrence.canonical)}</h3>
      {rows.length > occurrences.length && <p className="mb-4 flex items-center gap-2 text-sm font-medium text-accent"><LockKeyhole size={16} aria-hidden="true" />{rows.length - occurrences.length} ocorrências protegidas por Managed Values</p>}
    ...
    {rows.map(({ occurrence: o, display }) => <div key={o.id} className={"grid gap-6 xl:grid-cols-[1fr_1fr] " + (protectedOccurrence(o) ? "rounded-xl border border-accent/30 bg-accent/5 p-5" : "border-b py-5")}>
      ...
      {protectedOccurrence(o) ? <section aria-label="Gerenciamento deste campo" className="rounded-lg border border-accent/20 bg-white p-4">
      ...
      <button type="button" disabled={protectedOccurrence(o)} onClick={() => { setInputs({ ...inputs, [o.id]: editableValue(o.canonical) }); setReview(null); }} className="ui-btn ui-btn-ghost mt-2">Manter valor atual</button>
