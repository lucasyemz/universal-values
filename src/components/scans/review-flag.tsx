...
export function ReviewFlag({ scanId, pendingIds, reviewedIds, menu = false }: { menu?: boolean; scanId: string; pendingIds: string[]; reviewedIds: string[] }) {
  const t = useText();
  ...
  return <section aria-label={t("Marcação de revisão")} className={menu ? "border-b pb-2" : "mb-4 border-b pb-4"}>
    <p className={menu ? "mb-2 px-3 text-xs text-muted" : "mb-3 text-xs text-muted"}>{t("Nada a alterar? Marque como revisado. Isso não aplica os valores digitados.")}</p>
    <div className="flex flex-wrap items-center gap-3">
      {!!pendingIds.length && <button type="button" disabled={busy} onClick={() => void confirm(true, pendingIds)} className={menu ? "ui-nav-link w-full text-left" : "ui-btn"}><CheckCheck size={16} aria-hidden="true" />{busy ? t("Salvando…") : t("Marcar {0} como revisadas", pendingIds.length)}</button>}
