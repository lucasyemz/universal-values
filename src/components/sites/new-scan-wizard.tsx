...
import { ContextHelp, Steps } from "@/components/ui";
...
export function NewScanWizard({ siteId, operationId, collections, initialQuery, initialPlan }: { siteId: string; operationId: string; collections: { id: string; displayName: string }[]; initialQuery?: string; initialPlan?: Scan["plan"] }) {
  const t = useText();
        ...
        <form action={previewScan} className="space-y-6" onSubmit={event => { if (step === 0) { event.preventDefault(); next(event.currentTarget); } else if (!parseScanSetup(new FormData(event.currentTarget)).success) { event.preventDefault(); setError(t("Selecione pelo menos um tipo ou informe um texto válido para buscar.")); } }}>
          ...
          <div>
            <div hidden={step !== 0} className="space-y-6"><fieldset className="space-y-3"><legend className="mb-3 font-semibold">{t("Origem")}</legend>
              <input type="hidden" name="source" value="cms" /><div className="flex flex-wrap items-center gap-3"><p className="font-medium">{t("CMS Webflow")}</p><button type="button" disabled className="ui-btn cursor-not-allowed opacity-50">{t("SEO (em breve)")}</button></div><p className="text-sm text-muted">{t("Busque nos itens das coleções deste site.")}</p>
            ...
            <fieldset><legend className="mb-2 font-semibold">{t("Coleções")}</legend><p className="mb-3 text-xs text-muted">{t("Selecione de 1 a 20. Menos coleções tornam a leitura mais rápida.")}</p>
              <div className="max-h-72 space-y-2 overflow-y-auto p-1">{collections.length === 0 ? <p className="text-muted">{t("Nenhuma coleção disponível.")}</p> : collections.map((c) => <label key={c.id} className="ui-selection"><input type="checkbox" name="collectionIds" value={c.id} defaultChecked={initialPlan?.some(entry => entry.id === c.id)} />{c.displayName}</label>)}</div>
            </fieldset></div>
