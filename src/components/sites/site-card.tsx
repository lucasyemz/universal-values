...
export function SiteCard({ siteId,settings,fetchedAt,name, href, url, image, connected }: { siteId:string;settings:string;fetchedAt:string|null;name: string; href: string; url: string | null; image: string | null; connected: boolean }) {
 ...
 const links=[{href, label:t("Visão geral do site"),Icon:Globe2},{href:base+"/scans",label:t("Scans"),Icon:ScanLine},{href:base+"/variables",label:t("Variáveis"),Icon:Layers3},{href:base+"/changes",label:t("Histórico"),Icon:History},{href:base+"/cms",label:t("CMS Explorer"),Icon:Globe2},{href:settings,label:t("Configurações do Webflow"),Icon:Settings}];
