...
export function SitePage({ site, title, description, newScan = false, children }: { site: { id: string; workspace_id: string; display_name: string }; title: string; description: string; newScan?: boolean; children: ReactNode }) {
  ...
  return <main className="ui-page"><SiteContext title={title} siteName={site.display_name} siteId={site.id} workspaceId={site.workspace_id} /><PageHeader eyebrow={site.display_name} title={title} description={description} actions={newScan ? <Link href={`/dashboard/sites/${site.id}/scans/new`} className="ui-btn ui-btn-primary"><ScanLine size={16} aria-hidden="true" />{t("Novo scan")}</Link> : undefined} />{children}</main>;
}
