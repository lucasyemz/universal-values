...
export function SitePagination({ page, total, hasMore, base, query = {}, nextHref, previousHref }: { page: number; total?: number; hasMore?: boolean; base: string; query?: Record<string,string>; nextHref?:string; previousHref?:string }) {
  const t = useText();

  const href = (number: number) => base + "?" + new URLSearchParams({ ...query, page: String(number) });
  return <nav aria-label={t("Paginação")} className="mt-5 flex flex-wrap items-center justify-between gap-3 text-sm"><p className="text-muted tabular-nums">{t("Página")} {page}{total !== undefined ? t(" · {0} registros", total) : ""}</p><div className="flex gap-2">{page>1 && <Link className="ui-btn" href={previousHref ?? href(page-1)}><ArrowLeft size={14} />{t("Anterior")}</Link>}{(hasMore ?? (page*PAGE_SIZE<(total??0))) && <Link className="ui-btn" href={nextHref ?? href(page+1)}>{t("Próxima")}<ArrowRight size={14} /></Link>}</div></nav>;
}
