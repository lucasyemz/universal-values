...
describe("Designer static text adapter", () => {
  ...
  it("scans a static non-home page without calling the failing CMS lookup", async () => {
    ...
    f.api.getCurrentPage = async () => ({ ...page, isHomepage: async () => false, getCollectionId });
  ...
  it("still blocks a CMS template", async () => {
    const f = install();
    const page = await f.api.getCurrentPage();
    f.api.getCurrentPage = async () => ({ ...page, getKind: async () => "cms" });
    await expect(new DesignerTextPort().scan()).rejects.toThrow("não um template do CMS");
  });
  it.each(["ecommerce", "utility", "userSystems", "staticTemplate", "unknown"])("blocks unsupported page category %s", async kind => {
    const f = install();
    const page = await f.api.getCurrentPage();
    f.api.getCurrentPage = async () => ({ ...page, getKind: async () => kind });
    await expect(new DesignerTextPort().scan()).rejects.toThrow("Abra uma página estática");
    expect(f.text.getText).not.toHaveBeenCalled();
    expect(f.text.setText).not.toHaveBeenCalled();
  });
  it("does not assume a static page when classification fails", async () => {
    const f = install();
    const page = await f.api.getCurrentPage();
    f.api.getCurrentPage = async () => ({ ...page, getKind: async () => { throw new Error("Permission denied"); } });
    await expect(new DesignerTextPort().scan()).rejects.toThrow("page.getKind");
    expect(f.text.getText).not.toHaveBeenCalled();
  });
  it("resolves a fresh handle for the exact page when metadata returns Missing page", async () => {
    ...
    f.api.getCurrentPage = async () => ({ ...goodPage, getKind: async () => { throw new Error("Missing page page."); } });
