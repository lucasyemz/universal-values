...
function Extension() {
  ...
  return <main>
    ...
    {home && view === "home" && <>
      <div className="lab-grid"><section><span className="badge">DESIGNER</span><h2>Páginas estáticas</h2><p>Busque textos iguais e revise as mudanças na página aberta.</p><button className="primary" disabled={busy} onClick={() => setView("static")}>Buscar nesta página</button></section>
    ...
    <form onSubmit={event => { event.preventDefault(); void run(async () => {
      ...
      const { scan: result, mentions: found } = await controller.search(search);
    ...
    {scan && mentions.length > 0 && <section>
      ...
      <button disabled={busy || !Object.keys(replacements).length} className="primary" onClick={() => void run(async () => { invalidate(); setPlan(await controller.preview(scan, scannedTerm, replacements)); })}>Salvar prévia e revisar</button>
    </section>}
