...
export const changeRow = z.object({ id: z.uuid(), site_id: z.uuid(), status: z.string(), cursor: z.number(), total: z.number(), background_paused: z.boolean(), managed_value_id: z.string().nullable(), results: z.array(z.object({ status: z.string() }).passthrough()) });
...
export function changeActivity(row: z.infer<typeof changeRow>, site: string): Activity {
  ...
  const problem = row.results.some(r => !["applied", "already_applied"].includes(r.status));
  const state = pending ? row.background_paused ? "attention" : "active" : problem ? "attention" : "done";
  return { id: row.id, kind: "change", title: row.managed_value_id ? "Sincronização de Managed Value" : "Alteração no CMS", site, href: `/dashboard/changes/${row.id}`, state, label: pending ? row.background_paused ? "Aguardando revisão" : "Na fila / processando" : row.status === "cancelled" ? "Cancelada" : problem ? "Concluída com pendências" : "Concluída", detail: `${row.cursor} de ${row.total} campos processados`, current: row.cursor, total: row.total };
}
