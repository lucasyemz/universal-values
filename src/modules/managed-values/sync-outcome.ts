type Operation = { status: string; total: number; cursor: number; results: { status: string }[] };
export function syncOutcome(operation: Operation) {
  ...
  if (operation.status === "confirmed") return { verified, issues, remaining, badge: "paused", label: operation.cursor === 0 ? "Aplicação pendente" : "Aplicação incompleta" };
