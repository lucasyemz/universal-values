...
describe("background progress UI actions",()=>{
  it("only reads progress and health; never advances a field",async()=>{
    ...
    const rpc=vi.fn(async(name:string)=>({data:name==="operation_progress"?{cursor:1,total:2,status:"confirmed",verified:0,issues:1,paused:false,error:null,queuePosition:1,retryAt:null,updatedAt:new Date().toISOString()}:new Date().toISOString(),error:null}));
    ...
    expect(rpc).toHaveBeenCalledWith("cms_worker_last_seen",{});
