...
export async function readScanBatch(scan: Scan, siteId: string, reader: Pick<WebflowReader, "sites" | "collections" | "collection" | "items">) {
  ...
  const detected = detectPage(details, selectedPage, planned.types ?? ["money", "phone", "date", "number", "text"], planned.searchText, planned.searchOptions, planned.placeholders);
