...
export async function siteScansPage(id: string, page: number) {
  ...
  return { site, scans: z.array(scanSchema).parse(result.data), total: result.count ?? 0 };
}
export async function siteValuesPage(id: string, page: number, query: string, filter: string) {
  ...
  return { site, values, sources, total: result.count ?? 0 };
}
const operationSchema = z.object({ id: z.uuid(), status: z.string(), cursor: z.number(), total: z.number(), created_at: z.string(), expires_at: z.string(), results: z.unknown(), managed_value_id: z.string().nullable(), reverts_request_id: z.string().nullable() });
...
export async function siteChangesPage(id: string, page: number, filter: string) {
  ...
  const [cms, designer] = await Promise.all([
    filter === "static" ? null : client.from("cms_change_requests").select("id,status,cursor,total,created_at,expires_at,results,managed_value_id,reverts_request_id").eq("site_id", id).order("created_at", { ascending: false }).order("id").limit(filter === "attention" ? 1000 : limit),
  ...
  const rows: SiteActivity[] = z.array(operationSchema).parse(cms?.data ?? []).map(row => ({ id: row.id, source: "cms", title: row.reverts_request_id ? "Reversão" : row.managed_value_id ? "Sincronização de valor" : "Edição de conteúdo", target: "CMS", ...cmsOperationSummary(row), total: row.total, createdAt: row.created_at, href: `/dashboard/changes/${row.id}` }));
  ...
  return { site, rows: sorted.slice((page-1)*PAGE_SIZE,page*PAGE_SIZE), hasMore: sorted.length>page*PAGE_SIZE, limited: filter === "attention" };
}
export async function siteOverview(id: string) {
  ...
  const activity = [
    ...(scans.data ?? []).map(scan => ({ id: scan.id, title: "Scan do CMS", description: `${scan.occurrences_count} ocorrências registradas`, createdAt: scan.created_at, status: scan.status, label: undefined as string | undefined, href: `/dashboard/scans/${scan.id}` })),
  ...
  return { attention, activity, site, activeValues: values.count ?? 0, scanCount: scans.count ?? 0, scans: scans.data ?? [], running: running.data ?? [], uncertainCount: uncertain.count ?? 0, uncertain: uncertain.data ?? [], recent: recent.rows };
}
