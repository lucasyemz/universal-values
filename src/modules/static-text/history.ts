...
export function summarizeDesignerChange(input: { plan: unknown; events: unknown; expires_at: string }, now = Date.now()) {
  ...
  return { pageName: plan.context.pageName, changes, verified, status, searchDescription: plan.changes[0]?.image?"Imagens da página":plan.changes[0]?.link ? "Links da página" : searchOptionsLabel(plan.searchOptions) };
}
