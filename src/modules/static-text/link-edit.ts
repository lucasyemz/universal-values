...
const valueSchema=z.union([z.string(),z.discriminatedUnion("mode",[z.object({mode:z.literal("url"),to:z.string()}).passthrough(),z.object({mode:z.literal("page"),to:z.object({pageId:z.string()})}).passthrough()])]);
...
export function validLinkEdit(before:string,after:string,url:string,convertsPage?:boolean){try{
 ...
 return (convertsPage===undefined||convertsPage===(typeof value!=="string"&&value.mode==="page"))&&replaceLinkSnapshot(before,url)===after;
}catch{return false;}}
