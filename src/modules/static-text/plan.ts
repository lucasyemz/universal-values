import {imageReviewSchema,validImageEdit} from "./image-edit";
...
export const nodeSchema = z.object({ id: z.string().min(1), text: z.string().max(10000), location: z.string().max(2000).optional(), source: componentSourceSchema.optional() });
...
export const changeSchema = z.object({ id: z.string(), before: z.string().max(10000), after: z.string().max(10000), source: componentSourceSchema.optional(), link: linkReviewSchema.optional(), image:imageReviewSchema.optional() }).refine(change => !change.link || validLinkEdit(change.before, change.after, change.link.afterUrl, change.link.convertsPage), "Prévia de link inválida.").refine(change=>!(change.link&&change.image)&&(!change.image||validImageEdit(change.before,change.after,change.image)), "Prévia de imagem inválida.");
...
}).refine(plan => plan.changes.every(change => Boolean(change.link) === Boolean(plan.changes[0]?.link)&&Boolean(change.image)===Boolean(plan.changes[0]?.image)), "Prévia com tipos misturados.").refine(plan => new Set(plan.changes.map(change => change.id)).size === plan.changes.length);
export type TextPlan = z.infer<typeof planSchema>;
export type Mention = { key: string; nodeId: string; start: number; end: number; before: string; text: string; after: string; location?: string; source?: TextNode["source"] };
...
export function findMentions(nodes: TextNode[], input: string, options?: SearchOptions): Mention[] {
  ...
  for (const node of nodes) {
    for (const { start, end, raw } of findTextMatches(node.text, search, options)) {
      result.push({ key: JSON.stringify([node.id, start]), nodeId: node.id, start, end, ...(node.location ? {location:node.location} : {}), ...(node.source ? {source:node.source} : {}),
