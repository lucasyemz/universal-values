begin;
-- Numeric matches of an explicit search also begin pending in each new scan.
...
begin
  ...
  where o.scan_id=p_scan_id and case
    when exists(select 1 from jsonb_array_elements(s.plan) entry where entry->>'id'=o.collection_id and
      (o.canonical->>'type'='text' and length(btrim(coalesce(entry->>'searchText','')))>0
       or o.canonical->>'type'='number' and case when btrim(entry->>'searchText') ~ '^-?(0|[1-9][0-9]*)([.,][0-9]+)?$'
         then replace(btrim(entry->>'searchText'),',','.')::numeric=(o.canonical->>'number')::numeric else false end))
    then coalesce(
