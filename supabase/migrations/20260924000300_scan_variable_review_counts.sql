begin;
-- Exclude linked fields AFTER grouping, matching the scan tab partition.
-- Existing (site_id, source_key) binding index serves the anti-join; no new index.
...
create or replace function public.scan_review_summaries(p_ids uuid[]) returns table(scan_id uuid,pending bigint,reviewed bigint,total bigint,numeric_singletons jsonb)
...
begin
 ...
 foreach sid in array p_ids loop
  ...
  reviewed_ids as materialized (
   ...
   select c.* from classified c where not exists (select 1 from public.managed_value_bindings b where b.site_id=s.site_id and b.source_key=c.source_key)
  ), eligible as (
   select * from ordinary where n>=2 or canonical->>'type'='text' and exists(select 1 from jsonb_array_elements(s.plan) e where length(btrim(coalesce(e->>'searchText',''),U&'\0009\000A\000B\000C\000D\0020\00A0\1680\2000\2001\2002\2003\2004\2005\2006\2007\2008\2009\200A\2028\2029\202F\205F\3000\FEFF'))>0 or e->>'placeholders'='true')
  )
  select sid, count(*) filter(where not done),count(*) filter(where done),count(*),
   coalesce((select jsonb_agg(jsonb_build_object('collection_id',collection_id,'number',canonical->>'number','reviewed',done)) from ordinary where n=1 and canonical->>'type'='number'),'[]'::jsonb)
   from eligible;
 end loop;
end $$;
